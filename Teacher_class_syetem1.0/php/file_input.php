...
header("Content-type: text/html; charset=utf-8"); 
    ...
    if($_FILES['testFile']['name']=='')
    {
      ...
      echo"
          <script>alert('还没选择文件呢，你不是在逗我吧！');
          window.location.href='../html/teacher/teacher-index-input.php'</script>";

    }
    else if ($_FILES['testFile']['name']=='计算机专业.xls') {
        $table_name='teacher_class_computer_normal';
            }
    else if ($_FILES['testFile']['name']=='计算机（实验班）.xls') {
        $table_name='teacher_class_computer_operation';
            }
    else if ($_FILES['testFile']['name']=='计算机（卓越班）.xls') {
        $table_name='teacher_class_computer_excellent';
            }  
    else{
      echo"
          <script>alert('文件名错误!');
